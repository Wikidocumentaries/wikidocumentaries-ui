<template>
  <div v-if="results.length">
    <div class="gallery-component">
      ...
      <div class="intro">{{ $t('topic_page.Collection.intro') }}</div>
...
export default {
  ...
  mounted() {
    ...
    let sparql;
    sparql = `
SELECT ?item ?itemLabel (GROUP_CONCAT(DISTINCT ?typeLabel_; separator=", ") as ?typeLabel) (SAMPLE(?image) AS ?image) (GROUP_CONCAT(DISTINCT ?dated; separator="/") as ?time) (GROUP_CONCAT(DISTINCT ?creatorLabel_; separator=", ") as ?creatorLabel) (GROUP_CONCAT(DISTINCT ?creatorsexLabel_; separator=", ") as ?creatorsexLabel) WHERE {
  ?item wdt:P195 wd:Q2983474 .
