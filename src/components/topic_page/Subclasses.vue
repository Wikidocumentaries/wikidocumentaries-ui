...
<div v-if="results.length">
	<div class="gallery-component">
		<div class="toolbar">
            <div class="header-title">{{ $t('topic_page.Subclasses.headerTitle') }}</div>
        ...
        <div class="intro">{{ $t('topic_page.Subclasses.intro') }}</div>
        <div v-if="gallery" class="gallery">
            <!--img :src="wikidocumentaries.galleryImageURL" class="gallery-image"/-->
            <router-link tag="div" v-for="item in results" :key="item.id" :to="getItemURL(item.subclass.value)" class="gallery-item">
                ...
                <div :class="(item.image ? 'thumb-image-info' : 'thumb-image-info-plain')">
                    <div class="gallery-title">{{ item.subclass.label }}</div>
                    <div class="thumb-credit disappearing">{{ item.subclass.description }}</div>
                </div>
            </router-link>
        </div>
				<div v-else class="list">
            <div v-for="item in results" :key="item.id" class="listrow">
            <a :href="getItemURL(item.subclass.value)"><b>{{ item.subclass.label }}</b><i> {{ item.subclass.description }}</i>
            </a>
            </div>
        </div>
	</div>
...
const DEFAULT_SORT = ["subclass.label"];
...
export default {
    ...
    mounted() {
        ...
        let sparql;
        sparql = `
SELECT ?subclass ?subclassLabel ?subclassDescription (SAMPLE(?image) as ?image) WHERE {
  ?subclass wdt:P279 wd:Q89288125 .
  OPTIONAL { ?subclass wdt:P18 ?image. }
...
LIMIT 1000
        `.replace(/Q89288125/g, this.$store.state.wikidocumentaries.wikidataId)
