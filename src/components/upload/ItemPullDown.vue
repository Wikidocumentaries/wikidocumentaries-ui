...
<template>
    <div>
      <div v-if = "addedTerm"
        ...
        @click="openSearch"
      >{{ addedTerm }} <a :href="addedTermLink" target ="_blank">{{ ` (${addedTermId})`}}</a></div>
  ...
  export default {
    ...
    methods: {
      ...
      addTerm: function(topic){
        if (topic !== ''){
            this.addedTerm = topic.wikipage;
            this.addedTermId = topic.wikidata;
            this.addedTermLink = topic.wikidocumentarieslink;
            this.$emit('clicked', this.addedTerm)
        }
      },
